...
async def suggest_improvements(prompt_id: str) -> dict:
    ...
    # 현재 프롬프트 텍스트 조회
    from app.services.domains.proposal.prompt_registry import get_active_prompt
...
async def create_candidate(prompt_id: str, text: str, reason: str) -> Optional[int]:
    """후보 프롬프트 등록 (레지스트리 위임)."""
    from app.services.domains.proposal.prompt_registry import register_candidate
