...
# 발주기관명 기반 도메인 매핑 (새로운 PRIMARY 방식)
# 예: 철도연구원 → 교통/철도, 국방부 → 국방 등
# 이 맵에 없는 기관은 제목 기반 도메인 추출 사용 (FALLBACK)
AGENCY_DOMAIN_MAP: Dict[str, List[str]] = {
    # 교통물류 도메인
    "철도": ["교통", "철도"],
    "철도연구원": ["교통", "철도"],
    "해양": ["해양"],
    ...
    # 과학기술R&D 도메인 (여러 분야에 횡단적)
    "과학기술정보통신부": ["과학기술", "정보", "ICT"],
    "KIST": ["과학기술"],
    "KAIST": ["과학기술"],
    "ETRI": ["정보", "ICT"],
    "KISTI": ["과학기술", "정보"],
...
def score_bid(raw: Dict[str, Any], reference_date: Optional[date] = None) -> BidScore:
    ...
    for kw in DOMAIN_KEYWORDS:
        if kw.lower() in title_lower:
            if kw not in result.domain_keywords_matched:
                result.domain_keywords_matched.append(kw)

    # 핵심 도메인 체크 (agency 기반 도메인도 포함)
    core_matched = [kw for kw in result.domain_keywords_matched if kw in CORE_DOMAIN_KEYWORDS]
