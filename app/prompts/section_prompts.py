...
EVAL_ITEM_FALLBACK = """(평가항목 세부 배점이 RFP에서 추출되지 않았습니다.
일반적인 공공기관 제안 평가 기준으로 작성하세요:
- RFP 요구사항과의 정합성을 최우선 확인
- 구체적 근거(수치·사례)를 통한 설득력
- 실현 가능한 구체적 실행 방안 제시)"""
...
### 충실도 기준 (분량 미달 시 감점)
- **분량 가이드에 명시된 최소 페이지 수를 반드시 충족**하세요
- 구체적 근거(수치, 통계, 사례, 표, 도표)를 섹션당 최소 3개 이상 포함하세요
- 표(Table) 또는 다이어그램을 섹션당 최소 1개 포함하세요
- 추상적 서술("효율적으로 수행", "체계적으로 관리")만으로 채운 문단은 구체성 미달입니다
- 평가위원이 "이 업체는 얇게 써왔구나"라고 느끼면 중하위 점수 → 충분한 분량과 밀도 확보
"""
...
SECTION_PROMPT_UNDERSTAND = """'{section_id}' 섹션(사업의 이해)을 작성하세요.
...
SECTION_PROMPT_STRATEGY = """'{section_id}' 섹션(추진 전략)을 작성하세요.
...
SECTION_PROMPT_METHODOLOGY = """'{section_id}' 섹션(수행 방법론)을 작성하세요.
...
- 이해관계자 피드백 수집·반영 절차
- 예상치 못한 이슈 발생 시 의사결정 프로세스 (에스컬레이션 기준, 대안 수립 절차)
...
{{
  ...
  "self_check": {{
    ...
    "adaptive_management_included": true,
    "eval_sub_items_all_addressed": true,
    "depth_score": {{
      "evidence_count": 0,
      "tables_or_diagrams": 0,
...
SECTION_PROMPT_TECHNICAL = """'{section_id}' 섹션(기술적 수행방안)을 작성하세요.
...
SECTION_PROMPT_MANAGEMENT = """'{section_id}' 섹션(사업 관리)을 작성하세요.
...
SECTION_PROMPT_PERSONNEL = """'{section_id}' 섹션(투입 인력)을 작성하세요.
...
SECTION_PROMPT_TRACK_RECORD = """'{section_id}' 섹션(수행 실적)을 작성하세요.
...
SECTION_PROMPT_SECURITY = """'{section_id}' 섹션(보안 대책)을 작성하세요.
...
SECTION_PROMPT_MAINTENANCE = """'{section_id}' 섹션(유지보수/하자보수)을 작성하세요.
...
{{
  ...
  "self_check": {{
    ...
    "sustainability_plan_included": true,
    "eval_sub_items_all_addressed": true,
    "depth_score": {{
      "evidence_count": 0,
      "tables_or_diagrams": 0,
...
SECTION_PROMPT_ADDED_VALUE = """'{section_id}' 섹션(부가제안/기대효과)을 작성하세요.
...
2. **기대효과 (정량)** — SMART 기준으로 구체화, 산출 근거 포함
   - 아래 표 형식으로 정량 효과를 정리하세요:
   | 목표 | 현재(Baseline) | 목표치(Target) | 측정방법 | 달성시점 |
   - 예: "연간 OO건 수작업 → 자동화 시 인건비 OO원 절감"
3. **기대효과 (정성)** — 조직 역량 강화, 대국민 서비스 개선 등
4. **향후 확장 로드맵** — 본 사업 완료 후 발전 방향 + 자체 운영 전환 계획 포함
...
SECTION_PROMPT_CASE_B = """'{section_id}' 섹션을 **발주기관 지정 서식**에 맞춰 작성하세요.
...
def get_recommended_pages(eval_weight: int, total_pages: int = 100) -> str:
    """평가 배점 기반 권장 분량 산출 (최소/최대 범위 포함)."""
    if eval_weight <= 0:
        return "최소 2페이지 이상, 권장 2~3페이지"
    ratio = eval_weight / 100
    target = min(max(2, round(total_pages * ratio)), 15)
    min_pages = max(2, target - 1)
    max_pages = min(target + 2, 18)
    return (
        f"최소 {min_pages}페이지 이상, 권장 {target}페이지 (배점 {eval_weight}점 기준). "
        f"최대 {max_pages}페이지. {min_pages}페이지 미만은 구체성 부족으로 감점 대상."
    )
