...
## 핵심 규칙 (반드시 준수)
1. 고정 슬라이드(cover, agenda, key_message, closing)는 반드시 포함하라.
2. 평가항목 슬라이드는 score_weight 내림차순으로 배치하라.
3. 배점별 슬라이드 수: ≥20점→3~4장, 15~19점→2~3장, 10~14점→1~2장, ≤9점→1장.
4. 슬라이드 title은 "주어 + 서술어" 완결 주장 문장(assertion)으로 작성하라.
   예: ❌ "사업 이해도" → ✅ "당사는 현장 경험 5년으로 발주처 핵심 요구사항을 정확히 이해함"
   (단, cover/closing title은 예외)
5. 전체 흐름: 도입(문제 공감→Win Theme 선언) → 전개(배점 높은 순) → 결론(Win Theme 재확인).
6. 슬라이드 총 25~35장 (발표 15분 기준 슬라이드당 ~30초).
...
| 추진일정/타임라인 | timeline |
| 인력투입계획 | team |
| 핵심 수치/실적 강조 (최대 2회) | numbers_callout |
| 방법론/수행절차 | process_flow |
| 발주처 문제 공감 (key_message 직후) | problem_sync |
| 비전/슬로건 강조 (선택) | quote_highlight |
| 차별화 비교 | comparison |
| 유사 사례 증거 (정량 성과만) | case_study |

## 흐름 설계 원칙
- 앞 슬라이드가 다음 슬라이드의 질문을 유도하는 흐름: "문제 현황" → "왜 안 되는가?" → "해결책" → "증거"
- problem_sync는 key_message 직후 배치 — 발주처 문제 공감으로 신뢰 형성"""
...
## 핵심 규칙 (반드시 준수)
1. 모든 bullet의 근거는 proposal_sections에서만 추출하라. 새 내용 창작 금지.
2. Visual Brief의 composition(레이아웃)을 반드시 따르라. 임의 변경 금지.
3. highlight: Visual Brief의 highlight_element 그대로, 슬라이드 전체에서 1개만.
4. title: TOC의 assertion title 그대로 유지. 임의 변경 금지.
5. 6×6 규칙: bullets 최대 6개, 각 30자 이내 (sub 필드로 세부 내용 보완).
6. 정량 수치 bullet 슬라이드당 최소 1개. win_theme은 표지·key_message·closing에 명시.

## speaker_notes 구조 (4섹션)
[발표 스크립트] 평가위원이 이 항목에서 확인하는 것은 ~입니다. (2~3문장)
[예상 질문] Q: 질문 1~2개
[답변 구조] 결론 → 근거 2개 → 재확인 (30초)
[전환 멘트] 다음 슬라이드로 연결하는 한 문장

## 레이아웃별 필수 필드
| 레이아웃 | 필수 필드 |
|---------|----------|
| comparison | table + competitor_label |
| team | team_rows |
| numbers_callout | numbers [{value, label, description}] 3~4개 |
| process_flow | steps [{name, description, outputs}] 3~5단계 |
| problem_sync | current_state + pain_points + problem_statement |
| quote_highlight | quote + source + context |
| split_panel | chart_data + chart_title + f_pattern_anchor + points 2~4개 |
| numbered_strategy | points [{num, headline, sub_text, emphasis_value}] |
| case_study | cases [{client, project, challenge, solution, result, relevance}] 2~3개 |

## bullets 형식
str 또는 {"text": "주요 메시지 30자", "sub": "보조 설명"} 혼합 가능."""
