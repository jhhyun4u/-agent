  ...
  if (status === "processing" || status === "initialized") {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-500/15 text-blue-400 border border-blue-500/30 animate-pulse">
        <span className="w-1.5 h-1.5 rounded-full bg-blue-400" />처리중
      </span>
    );
  }
  if (status === "completed") {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-[#3ecf8e]/15 text-[#3ecf8e] border border-[#3ecf8e]/30">
        <span className="w-1.5 h-1.5 rounded-full bg-[#3ecf8e]" />완료
      </span>
    );
  }
  if (status === "failed") {
    return (
      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-red-500/15 text-red-400 border border-red-500/30">
        <span className="w-1.5 h-1.5 rounded-full bg-red-400" />실패
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-[#262626] text-[#8c8c8c] border border-[#262626]">
      <span className="w-1.5 h-1.5 rounded-full bg-[#8c8c8c]" />초기화
    </span>
  );
}
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[#0f0f0f] text-[#ededed]">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* 우측 패널 — lg 이상 인라인 표시 */}
        {rightPanelOpen && (
          <aside className="w-[420px] shrink-0 border-l border-[#262626] bg-[#111111] overflow-y-auto hidden lg:block">
            <DetailRightPanel
              proposalId={id}
              status={status.status}
              phasesCompleted={status.phases_completed ?? 0}
      ...
      <div className={`lg:hidden fixed inset-0 z-40 ${rightPanelOpen ? "" : "hidden"}`}>
        ...
        <aside className="absolute right-0 inset-y-0 w-[85vw] max-w-[420px] bg-[#111111] overflow-y-auto">
          <DetailRightPanel
            proposalId={id}
            status={status.status}
            phasesCompleted={status.phases_completed ?? 0}
