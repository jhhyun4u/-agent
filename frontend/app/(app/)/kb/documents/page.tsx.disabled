 ...
 * 1. File type validation - clear error messages
 * 2. Filename search - ilike filtering
 * 3. Sort parameter - created_at, updated_at, filename, total_chars
 * 4. Reprocess guard - prevent concurrent processing
 * 5. Delete endpoint - full document removal
 */
...
export default function DocumentsPage() {
  ...
  }, [search, statusFilter, docTypeFilter, sortBy, sortOrder, offset]);

  useEffect(() => {
    setOffset(0);
  }, [search, statusFilter, docTypeFilter, sortBy, sortOrder]);
  ...
  // Handle reprocess with guard
  async function handleReprocess(documentId: string, currentStatus: string) {
    // Check if already in progress
    if (["extracting", "chunking", "embedding"].includes(currentStatus)) {
      alert("문서가 현재 처리 중입니다. 처리 완료 후 재시도하세요.");
      return;
    }

    if (reprocessConfirm !== documentId) {
      setReprocessConfirm(documentId);
      return;
    }
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold">인트라넷 문서</h1>
        <p className="text-gray-600 mt-1">문서 업로드 및 관리</p>
      </div>

      {/* Upload Section */}
      <Card className="border-blue-200 bg-blue-50">
        <div className="p-6">
          <h3 className="text-lg font-semibold mb-4">새 문서 업로드</h3>
      ...
      <Card>
        <div className="p-6">
          <h3 className="text-lg font-semibold mb-4">필터 및 정렬</h3>
