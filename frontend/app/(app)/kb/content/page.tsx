...
export default function ContentPage() {
  ...
  const [form, setForm] = useState<KbContentCreate>({
    title: "",
    body: "",
    type: "section_block",
    tags: [],
  ...
  function statusBadge(status: string) {
    ...
    return (
      <span
        className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${m[status] ?? m.draft}`}
      >
        {status}
      </span>
    );
    ...
    <>
      <header className="bg-[#111111] border-b border-[#262626] px-6 py-3 shrink-0 flex items-center justify-between">
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-[#3ecf8e] text-[#0f0f0f] hover:bg-[#3ecf8e]/90 transition-colors"
        >
          + 콘텐츠 등록
        </button>
      </header>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-3 text-[10px] text-[#8c8c8c]">
                    ...
                    {item.quality_score != null && (
                      <span>품질: {item.quality_score}점</span>
                    )}
                    {item.reuse_count > 0 && (
                      <span>재사용: {item.reuse_count}회</span>
                    )}
                ...
                <div className="flex gap-1.5 shrink-0">
                  {item.status === "draft" && (
                    <button
                      ...
                      className="px-2.5 py-1 text-[10px] rounded border border-[#3ecf8e]/30 text-[#3ecf8e] hover:bg-[#3ecf8e]/10 transition-colors"
                    >
                      승인
                    </button>
                  )}
