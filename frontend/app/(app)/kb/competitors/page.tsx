      ...
      <main className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        ...
        <div className="flex gap-2">
          {SCALE_OPTS.map((s) => (
            <button
              key={s.value}
              onClick={() => setFilterScale(s.value)}
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, primary_area: e.target.value })
                  }
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {item.scale && (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-[#262626] text-[#8c8c8c]">
                        {item.scale}
                      </span>
                    )}
                  </div>
                  <div className="flex gap-3 text-[10px] text-[#8c8c8c]">
                    ...
                    {item.price_pattern && (
                      <span>가격패턴: {item.price_pattern}</span>
                    )}
                    {item.avg_win_rate != null && (
                      <span>
                        수주율: {Math.round(item.avg_win_rate * 100)}%
                      </span>
                    )}
                  </div>
                </div>
