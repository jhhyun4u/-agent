      ...
      <main className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {RESULT_OPTS.map((r) => (
            <button
              key={r.value}
              onClick={() => setFilterResult(r.value)}
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-[#1c1c1c] border border-[#262626] rounded-xl overflow-hidden"
              >
                <button
                  onClick={() => toggleDetail(item.id)}
                  className="w-full px-4 py-3 flex items-center gap-3 text-left hover:bg-[#262626]/30 transition-colors"
                >
                  <svg
                    className={`w-3 h-3 text-[#8c8c8c] shrink-0 transition-transform ${expanded === item.id ? "rotate-90" : ""}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-sm font-medium text-[#ededed] truncate">
                        {item.strategy_summary || "교훈"}
                      </span>
                      {item.result && (
                        <span
                          className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${RESULT_BADGE[item.result] ?? "bg-[#262626] text-[#8c8c8c]"}`}
                        >
                          {item.result}
                        </span>
                      )}
                    ...
                    <div className="flex gap-3 text-[10px] text-[#8c8c8c]">
                      ...
                      {item.failure_category && (
                        <span>실패: {item.failure_category}</span>
                      )}
                      <span>
                ...
                {expanded === item.id && detail && (
                  <div className="px-4 pb-4 pt-0 space-y-2 border-t border-[#262626]">
                    {detail.effective_points && (
                      <div className="pt-3">
                        <p className="text-[10px] text-[#3ecf8e] mb-1 uppercase tracking-wider font-medium">
                          효과적이었던 점
                        </p>
                    ...
                    {detail.weak_points && (
                      <div>
                        <p className="text-[10px] text-amber-400 mb-1 uppercase tracking-wider font-medium">
                          부족했던 점
                        </p>
                        <p className="text-xs text-[#ededed] leading-relaxed">
                          {detail.weak_points}
                    ...
                    {detail.improvements && (
                      <div>
                        <p className="text-[10px] text-blue-400 mb-1 uppercase tracking-wider font-medium">
                          개선 사항
                        </p>
                        <p className="text-xs text-[#ededed] leading-relaxed">
                          {detail.improvements}
                    ...
                    {detail.failure_detail && (
                      <div>
                        <p className="text-[10px] text-red-400 mb-1 uppercase tracking-wider font-medium">
                          실패 상세
                        </p>
                        <p className="text-xs text-[#ededed] leading-relaxed">
                          {detail.failure_detail}
