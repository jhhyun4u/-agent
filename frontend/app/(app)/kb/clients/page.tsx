...
export default function ClientsPage() {
  ...
  const [form, setForm] = useState<KbClientCreate>({
    client_name: "",
    ...
    <>
      <header className="bg-[#111111] border-b border-[#262626] px-6 py-3 shrink-0 flex items-center justify-between">
        ...
        <button
          ...
          className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-[#3ecf8e] text-[#0f0f0f] hover:bg-[#3ecf8e]/90"
        >
          + 기관 등록
        </button>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
        ...
        {showForm && (
          <form
            ...
            className="bg-[#1c1c1c] border border-[#262626] rounded-xl p-4 space-y-3"
          >
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                >
                  {REL_OPTS.filter((r) => r.value).map((r) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  type="text"
                  value={form.eval_tendency ?? ""}
                  onChange={(e) =>
                    setForm({ ...form, eval_tendency: e.target.value })
                  }
                  placeholder="기술 중시/가격 중시"
                  className="w-full bg-[#111111] border border-[#262626] rounded-lg px-3 py-2 text-sm text-[#ededed] placeholder-[#5c5c5c] focus:outline-none"
                />
              </div>
              <div>
            ...
            <div className="flex gap-2">
              <button
                type="submit"
                disabled={saving}
                className="px-4 py-2 text-xs font-semibold rounded-lg bg-[#3ecf8e] text-[#0f0f0f] disabled:opacity-50"
              >
                {saving ? "저장 중..." : "저장"}
              </button>
              <button
                ...
                className="px-4 py-2 text-xs rounded-lg bg-[#262626] text-[#8c8c8c]"
              >
                취소
              </button>
            </div>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-[#1c1c1c] border border-[#262626] rounded-xl px-4 py-3 flex items-center gap-3"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${REL_BADGE[item.relationship] ?? REL_BADGE.neutral}`}
                    >
                      {item.relationship}
                    </span>
                  </div>
                  <div className="flex gap-3 text-[10px] text-[#8c8c8c]">
                    ...
                    {item.eval_tendency && (
                      <span>평가: {item.eval_tendency}</span>
                    )}
                ...
                <button
                  ...
                  className="px-2.5 py-1 text-[10px] rounded border border-red-500/30 text-red-400 hover:bg-red-500/10 shrink-0"
                >
                  삭제
                </button>
              </div>
