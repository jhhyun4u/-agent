...
import {
  ...
  normalizePromptId,
} from "@/components/prompt/promptLabels";
...
const SEED_DASHBOARD: PromptDashboard = {
  ...
  prompts: [
    ...SEED_SECTION_TYPES.map((id) => ({
      ...
      version: 1,
      source_file: "section_prompts.py",
      status: "active",
      content_hash: "-",
      metadata: {},
    ...
    {
      prompt_id: "strategy.positioning",
      version: 1,
    ...
    {
      prompt_id: "self_review",
      version: 1,
      source_file: "proposal_prompts.py",
      status: "active",
      content_hash: "-",
      metadata: {},
      created_at: "2026-03-01",
  ...
  effectiveness: [
    ...
    },
    {
      ...
      avg_input_tokens: 5100,
      avg_output_tokens: 2200,
      avg_duration_ms: 4100,
    },
    {
      prompt_id: "section.METHODOLOGY",
      prompt_version: 1,
      proposals_used: 10,
      won: 7,
      lost: 3,
    ...
    },
    {
      ...
      avg_input_tokens: 5500,
      avg_output_tokens: 2500,
      avg_duration_ms: 4500,
    },
    {
      prompt_id: "section.PERSONNEL",
      prompt_version: 1,
      proposals_used: 9,
      won: 6,
      lost: 3,
  ...
  edit_stats: [
    ...
    },
    {
      ...
      edit_count: 8,
      avg_edit_ratio: 0.35,
      actions: { rewrite: 2, refine: 6 },
    },
    {
      prompt_id: "section.MAINTENANCE",
      edit_count: 12,
      avg_edit_ratio: 0.55,
      actions: { rewrite: 6, refine: 6 },
    },
  ],
  running_experiments: [],
};
                  ...
                  <tbody>
                    {filteredPrompts.map((p) => {
                      ...
                      const catId = categories.find((c) =>
                        c.prompts.some((cp) => cp.prompt_id === p.prompt_id),
                      )?.id;
                      const catLabel =
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2 pr-4">
                            {catLabel ? (
                              <span
                                ...
                                style={{
                                  backgroundColor: `${catStyle?.color ?? "#666"}20`,
                                  color: catStyle?.color ?? "#666",
                                }}
                              >
                                {catLabel}
                              </span>
                          ...
                          <td className="text-right pr-4">
                            {eff?.win_rate != null ? `${eff.win_rate}%` : "-"}
                          </td>
                          <td className="text-right pr-4">
                            <span
                              className={
                            ...
                            >
                              {es
                                ? `${(es.avg_edit_ratio * 100).toFixed(0)}%`
                                : "-"}
                            </span>
                          </td>
                          <td className="text-right">
                            {eff?.avg_quality_score?.toFixed(0) ?? "-"}
                          </td>
                        </tr>
                      );
                    })}
                  ...
                  {heatmap.map((h) => (
                    ...
                    >
                      ...
                      <div className="text-[#8c8c8c] text-xs mt-1">
                        사용 {h.usage_count}회 | 품질{" "}
                        {h.avg_quality?.toFixed(0) ?? "-"}
                      </div>
                    </div>
                  ))}
                </div>
