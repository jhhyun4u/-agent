...
export default function PromptDetailPage() {
  ...
  const [effectiveness, setEffectiveness] = useState<PromptEffectiveness | null>(null);
  ...
  const fetchData = useCallback(async () => {
    ...
    if (detailRes.status === "fulfilled") {
      ...
      const activeV = detailRes.value.versions.find((v) => v.status === "active");
  ...
  const handleSaveCandidate = async () => {
    ...
    setSaving(true);
    try {
      const result = await api.prompts.createCandidate(promptId, editText, editReason);
      if (result.version) {
  ...
  const handleCreateCandidate = async (text: string, title: string) => {
    try {
      const result = await api.prompts.createCandidate(promptId, text, `AI 제안: ${title}`);
      if (result.version) {
        alert(`후보 v${result.version} 등록 완료`);
        fetchData();
      }
          ...
          <>
            ...
            {effectiveness && (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                <MetricCard label="사용 횟수" value={effectiveness.proposals_used} />
                <MetricCard label="승률" value={effectiveness.win_rate != null ? `${effectiveness.win_rate}%` : "-"} />
                <MetricCard label="평균 품질" value={effectiveness.avg_quality_score?.toFixed(1) ?? "-"} />
                <MetricCard
                  label="수정율"
                  value={effectiveness.avg_edit_ratio != null ? `${(effectiveness.avg_edit_ratio * 100).toFixed(1)}%` : "-"}
                />
                <MetricCard label="평균 토큰" value={`${effectiveness.avg_input_tokens ?? 0}/${effectiveness.avg_output_tokens ?? 0}`} />
              </div>
            )}
            ...
            <div className="flex gap-1 bg-[#1c1c1c] rounded-lg p-1 w-fit">
              {([
                { id: "history" as Tab, label: `이력 (${detail.total_versions})` },
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-md text-xs font-medium transition-colors ${
                    tab === t.id ? "bg-[#262626] text-[#ededed]" : "text-[#8c8c8c] hover:text-[#ededed]"
                  }`}
                >
                  {t.label}
                </button>
                ...
                {detail.versions.map((v) => (
                  <div
                    ...
                    className={`rounded-lg border p-3 ${
                      v.status === "active" ? "border-[#3ecf8e] bg-[#111]"
                        : v.status === "candidate" ? "border-[#f5a623] bg-[#111]"
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded ${
                          v.status === "active" ? "bg-[#1a3a2a] text-[#3ecf8e]"
                            : v.status === "candidate" ? "bg-[#3a2a1a] text-[#f5a623]"
                            : "bg-[#1a1a1a] text-[#8c8c8c]"
                        }`}>
                          {v.status}
                        </span>
                        <span className="text-xs text-[#8c8c8c]">
                          {v.created_by} &middot; {new Date(v.created_at).toLocaleDateString("ko")}
                        </span>
                      </div>
                      <button
                    ...
                    {v.change_reason && <div className="text-xs text-[#8c8c8c] mt-1">{v.change_reason}</div>}
            ...
            {tab === "suggest" && (
              <section className="bg-[#1c1c1c] rounded-2xl border border-[#262626] p-5">
                ...
                {suggestions?.error && <div className="text-xs text-[#ff6b6b]">{suggestions.error}</div>}
                {suggestions?.analysis && <div className="text-xs text-[#8c8c8c] mb-4">{suggestions.analysis}</div>}

                {suggestions?.suggestions && (
                  <div className="space-y-4">
                    {suggestions.suggestions.map((s, i) => (
                      <div key={i} className="bg-[#111] rounded-lg border border-[#262626] p-4">
                        <div className="flex items-center justify-between mb-2">
                          <h3 className="text-xs font-semibold">{s.title}</h3>
                          <div className="flex gap-2">
                            <button
                              onClick={() => handleApplySuggestion(s.prompt_text)}
                              className="px-3 py-1 bg-[#262626] hover:bg-[#333] rounded text-xs transition-colors"
                            >
                              편집기 적용
                            </button>
                            <button
                              onClick={() => handleCreateCandidate(s.prompt_text, s.title)}
                        ...
                        <p className="text-xs text-[#8c8c8c] mb-2">{s.rationale}</p>
                        <div className="flex flex-wrap gap-1">
                          {s.key_changes.map((c, j) => (
                            <span key={j} className="text-xs bg-[#1a1a1a] px-2 py-0.5 rounded text-[#8c8c8c]">{c}</span>
                          ))}
                        </div>
                      </div>
                    ))}
