...
export default function PositioningImpactModal({
    ...
    api.workflow
      ...
      .catch(() =>
        setImpact({
          ...
          message:
            "영향 범위를 확인할 수 없습니다. 전체 재생성이 필요할 수 있습니다.",
        }),
      )
  ...
  const cur = POS_LABELS[currentPositioning] ?? {
    label: currentPositioning,
    icon: "?",
  };
  const nxt = POS_LABELS[newPositioning] ?? {
    label: newPositioning,
    icon: "?",
  };
        ...
        <div className="bg-[#1c1c1c] border border-[#333] rounded-2xl w-full max-w-md shadow-2xl">
          ...
          <div className="px-5 py-4 space-y-4">
            ...
            <div className="bg-[#111111] border border-[#262626] rounded-lg px-4 py-3">
              <p className="text-[10px] text-[#8c8c8c] uppercase tracking-wider mb-2">
                영향 받는 항목
              </p>
              <ul className="space-y-1">
                {items.map((item, i) => (
                  <li
                    key={i}
                    className="text-xs text-[#ededed] flex items-center gap-2"
                  >
