...
export default function ProjectArchivePanel({ proposalId, className = "" }: Props) {
  ...
  async function handleSnapshot() {
    ...
    try {
      ...
      setSnapshotResult("스냅샷 실패: " + (e instanceof Error ? e.message : "알 수 없는 오류"));
    } finally {
  ...
  const sortedCategories = Object.keys(CATEGORY_LABELS).filter((k) => categories[k]?.length);
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[#8c8c8c] mt-0.5">
            {manifest?.total_count || 0}개 파일 / {formatBytes(manifest?.total_size || 0)}
          </p>
        </div>
      ...
      {sortedCategories.length === 0 ? (
        <div className="text-center py-12 text-[#8c8c8c] text-sm">
          <p>아카이브된 산출물이 없습니다.</p>
          <p className="text-xs mt-1">&quot;스냅샷 생성&quot;을 눌러 현재 워크플로 산출물을 파일화하세요.</p>
        </div>
      ) : (
        sortedCategories.map((cat) => (
          <div key={cat} className="border border-[#262626] rounded-lg overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="inline-flex items-center justify-center w-9 h-5 rounded text-[9px] font-mono font-bold bg-[#262626] text-[#8c8c8c] shrink-0">
                      {FORMAT_ICONS[file.file_format] || file.file_format?.toUpperCase()}
                    </span>
                    <span className="text-xs text-[#ededed] truncate">
                      {file.title}
                    </span>
                    {file.version > 1 && (
                      <span className="text-[9px] text-[#8c8c8c]">v{file.version}</span>
                    )}
                  </div>
