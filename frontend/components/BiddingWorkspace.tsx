...
const SUBMISSION_STATUS: Record<string, { label: string; color: string }> = {
  ...
  verified: {
    label: "검증 완료",
    color: "bg-[#3ecf8e]/15 text-[#3ecf8e] border-[#3ecf8e]/30",
  },
};
    ...
    return (
      <div className="max-w-3xl mx-auto bg-[#1c1c1c] border border-[#262626] rounded-xl p-8 text-center">
        ...
        <p className="text-xs text-[#8c8c8c] mt-1">
          워크플로에서 입찰가격 계획(STEP 2.5)이 완료되면 표시됩니다.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-[#1c1c1c] border border-[#262626] rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-medium text-[#8c8c8c] uppercase tracking-wider">
              확정 입찰가
            </h3>
            <button
          ...
          {workspace.market_summary &&
            workspace.market_summary.win_probability > 0 && (
              <div className="mt-3 flex items-center gap-2">
                <div className="flex-1 h-1.5 bg-[#262626] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${workspace.market_summary.win_probability}%`,
                    }}
                  />
                </div>
        ...
        {/* 투찰 상태 */}
        <div className="bg-[#1c1c1c] border border-[#262626] rounded-xl p-5">
          <h3 className="text-xs font-medium text-[#8c8c8c] uppercase tracking-wider mb-3">
            투찰 상태
          </h3>
          {subBadge ? (
            <span
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border ${subBadge.color}`}
            >
              {subBadge.label}
            </span>
          ) : (
          ...
          <div className="mt-3 flex items-center gap-3 text-xs text-[#8c8c8c]">
            {workspace.market_summary && (
              <>
                <span>
                  유사과제 {workspace.market_summary.comparable_cases}건
                </span>
                {workspace.market_summary.market_avg_ratio && (
                  <span>
                    시장평균 {workspace.market_summary.market_avg_ratio}%
                  </span>
                )}
      ...
      {workspace.scenarios && workspace.scenarios.length > 0 && (
        <div>
          <h3 className="text-xs font-medium text-[#8c8c8c] uppercase tracking-wider mb-2">
            시나리오 비교
          </h3>
          <div className="grid grid-cols-3 gap-3">
            {workspace.scenarios.map(
              (scenario: Record<string, unknown>, idx: number) => {
                const name = (scenario.name ||
                  scenario.label ||
                  `시나리오 ${idx + 1}`) as string;
                const isSelected =
                  name === bs?.bid_confirmed_scenario ||
                  (scenario.name as string) === bs?.bid_confirmed_scenario;
                return (
                  <div
                    key={idx}
          ...
          <div className="bg-[#1c1c1c] border border-[#262626] rounded-xl overflow-hidden">
            <div className="divide-y divide-[#262626]/50">
              {workspace.price_history.map((entry: BidPriceHistoryEntry) => {
                const evt = EVENT_LABELS[entry.event_type] || {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`text-xs font-medium w-16 shrink-0 ${evt.color}`}
                    >
                      {evt.label}
                    </span>
                    ...
                    {entry.reason && (
                      <span className="text-[10px] text-[#8c8c8c] truncate flex-1">
                        {entry.reason}
                      </span>
                    )}
                    {entry.actor_name && (
