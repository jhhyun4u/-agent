...
const NL: Record<string, string> = {
  ...
  plan_team: "팀구성", plan_assign: "역할배분", plan_schedule: "일정", plan_story: "스토리라인",
  plan_price: "예산산정", proposal_write_next: "섹션 작성", self_review: "자가진단",
  presentation_strategy: "발표전략", ppt_toc: "PPT 목차", ppt_visual_brief: "시각설계", ppt_storyboard: "슬라이드",
  bid_plan: "입찰가 결정", cost_sheet_generate: "산출내역서",
  submission_plan: "제출서류 계획", submission_checklist: "제출서류 확인",
  mock_evaluation: "모의 평가", eval_result: "평가결과", project_closing: "Closing",
};

const EST: Record<string, number> = {
  "RFP 분석": 120, "전략 수립": 180, "제안 계획": 300, "제안서 작성": 480, "PPT 생성": 180,
  "모의 평가": 120, "제출서류 계획": 60, "입찰가 결정": 120, "산출내역서": 60, "제출서류 확인": 60,
  "평가결과·Closing": 60,
};
  ...
  return (
    <div className={`bg-[#1c1c1c] rounded-2xl border border-[#262626] p-4 ${className}`}>
      ...
      <div className="flex items-center gap-3 mt-3 pt-2 border-t border-[#262626] flex-wrap text-[9px] text-[#8c8c8c]">
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[#3ecf8e]" />완료</span>
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-[#3ecf8e]/30 border border-[#3ecf8e]" />진행</span>
        <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-amber-500/30 border border-amber-500" />승인대기</span>
...
function Sub({ idx, np, cn }: { idx: number; np?: Map<string, NodeProgress>; cn: string }) {
  ...
  return (
    <div className="mt-3 pt-3 border-t border-[#262626]">
      <p className="text-[10px] text-[#8c8c8c] mb-2">{s.stepLabel} {s.label} 세부</p>
          ...
          return (
            <div key={n} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] ${
              ...
              {c ? <div className="w-2 h-2 rounded-full border border-[#3ecf8e] border-t-transparent animate-spin" />
               : d ? <span>✓</span> : <span className="w-1.5 h-1.5 rounded-full bg-[#363636]" />}
              {NL[n] || n}
            </div>
          );
        })}
