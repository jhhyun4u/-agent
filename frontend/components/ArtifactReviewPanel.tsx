...
const ARTIFACT_MAP: Record<string, ArtifactConfig> = {
  review_search: {
    title: "공고 검색 결과",
    artifactKey: "search_results",
    icon: "S",
    step: 1,
  },
  review_rfp: {
    title: "RFP 분석 결과",
    artifactKey: "rfp_analyze",
    icon: "R",
    step: 1,
  },
  review_gng: {
    title: "Go/No-Go 판정",
    artifactKey: "go_no_go",
    icon: "G",
    step: 1,
  },
  review_strategy: {
  ...
  review_ppt: {
    title: "PPT 발표자료",
    artifactKey: "ppt_storyboard",
    icon: "T",
    step: 5,
  },
  // v4.0: 분기 워크플로 신규 리뷰 게이트
  review_submission_plan: {
    title: "제출서류 계획",
    artifactKey: "submission_plan",
    icon: "F",
    step: 3,
  },
  review_cost_sheet: {
    title: "산출내역서",
    artifactKey: "cost_sheet",
    icon: "C",
    step: 5,
  },
  review_submission: {
    title: "제출서류 확인",
    artifactKey: "submission_checklist",
    icon: "V",
    step: 6,
  },
  review_mock_eval: {
    title: "모의 평가",
    artifactKey: "mock_evaluation",
    icon: "E",
    step: 6,
  },
  review_eval_result: {
    title: "평가결과",
    artifactKey: "eval_result",
    icon: "7",
    step: 7,
  },
};
...
export default function ArtifactReviewPanel({
  ...
  const [artifactData, setArtifactData] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const iconColor = ICON_COLORS[config.icon] ?? {
    bg: "bg-[#262626]",
    text: "text-[#8c8c8c]",
  };
  ...
  const sectionProgress =
    ...
    workflowState.total_sections != null
      ? {
          current: workflowState.current_section_index + 1,
          total: workflowState.total_sections,
        }
      : null;
...
function ArtifactContent({
  ...
  switch (artifactKey) {
    case "go_no_go":
      return <GoNoGoContent data={data} />;
    case "rfp_analyze":
      return <RfpAnalyzeContent data={data} />;
    case "search_results":
      return <SearchResultsContent data={data} />;
    case "strategy":
      return <StrategyContent data={data} />;
    case "bid_plan":
      return <BidPlanContent data={data} />;
    case "plan":
      return <PlanContent data={data} />;
    case "proposal":
      return <ProposalContent data={data} proposalId={proposalId} />;
    case "self_review":
      return <SelfReviewContent data={data} />;
    case "ppt_storyboard":
      return <PptStoryboardContent data={data} />;
    default:
...
function Section({
  ...
  return (
    <div className={`border ${border[variant]} rounded-xl p-3.5 mb-3`}>
      <p
        className={`text-[10px] font-semibold uppercase tracking-wider mb-2 ${titleColor[variant]}`}
      >
        {title}
      </p>
      {children}
...
function KV({
  ...
  return (
    <div className="flex items-center justify-between py-1">
      ...
      <span className={`text-[11px] font-medium ${cls ?? "text-[#ededed]"}`}>
        {value}
      </span>
    </div>
  );
...
function Tag({
  ...
  const m = {
    default: "bg-[#262626] text-[#8c8c8c]",
    green: "bg-[#3ecf8e]/10 text-[#3ecf8e]",
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-medium ${m[color]}`}
    >
      {children}
    </span>
  );
...
function GoNoGoContent({ data }: { data: Record<string, unknown> }) {
  ...
  const axes = breakdown
    ? [
        {
          label: "유사실적",
          score: breakdown.similar_performance ?? 0,
          max: 30,
        },
    ...
    <div>
      <div className="flex items-center gap-4 mb-4 pb-4 border-b border-[#262626]">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {recommendation && (
            <Tag color={recommendation === "go" ? "green" : "red"}>
      ...
      {fatalFlaw && (
        <Section title="치명적 결격 사유" variant="danger">
          <p className="text-xs text-[#ededed] leading-relaxed">{fatalFlaw}</p>
        </Section>
      )}
      {strategicFocus && (
        <Section title="핵심 승부수" variant="success">
      ...
      {pros && pros.length > 0 && (
        <Section title="강점">
          <Bullets items={pros} color="text-[#3ecf8e]" />
        </Section>
      )}
      {risks && risks.length > 0 && (
        <Section title="리스크" variant="warning">
          <Bullets items={risks} color="text-amber-400" />
        </Section>
      )}
    </div>
  );
    ...
    <div>
      ...
      {scope && (
        <Section title="프로젝트 범위">
          <p className="text-xs text-[#ededed] leading-relaxed">{scope}</p>
        </Section>
      )}
          ...
          <div className="space-y-1.5">
            {evalItems.map((e, i) => (
              ...
              >
                ...
                <span className="text-[11px] font-bold text-[#3ecf8e]">
                  {e.weight}점
                </span>
              </div>
            ))}
      ...
      {mandatoryReqs && mandatoryReqs.length > 0 && (
        <Section title="필수 요건" variant="danger">
          <Bullets items={mandatoryReqs} color="text-red-400" />
        </Section>
      )}
    </div>
  );
    ...
    <div>
      {winTheme && (
        <Section title="Win Theme" variant="success">
          <p className="text-sm font-medium text-[#ededed] leading-relaxed">
            {winTheme}
          </p>
        </Section>
      )}
      {ghostTheme && (
        <Section title="Ghost Theme">
          <p className="text-xs text-[#8c8c8c] leading-relaxed">{ghostTheme}</p>
        </Section>
      )}
          ...
          <ol className="space-y-1.5">
            {keyMessages.map((m, i) => (
              <li key={i} className="flex gap-2 text-[11px]">
                <span className="shrink-0 w-5 h-5 rounded-full bg-[#3ecf8e]/10 text-[#3ecf8e] flex items-center justify-center text-[10px] font-bold">
                  {i + 1}
                </span>
                <span className="text-[#ededed] leading-relaxed pt-0.5">
                  {m}
                </span>
              </li>
            ))}
          ...
          <div className="grid grid-cols-2 gap-2">
            {(
              ...
              const cfg: Record<string, { l: string; c: string }> = {
                strengths: { l: "S", c: "text-[#3ecf8e]" },
                weaknesses: { l: "W", c: "text-red-400" },
                opportunities: { l: "O", c: "text-blue-400" },
                threats: { l: "T", c: "text-amber-400" },
              };
              const { l, c } = cfg[key];
              return (
      ...
      {alternatives && alternatives.length > 1 && (
        <Section title={`전략 대안 (${alternatives.length}개)`}>
          {alternatives.map((a) => (
            <div
              key={a.alt_id}
              className="bg-[#0f0f0f] rounded-lg px-3 py-2 mb-1.5"
            >
    ...
    <div>
      ...
      {storylines && (storylines as any).sections && (
        <Section title="섹션별 스토리">
          <div className="space-y-2">
            {(
              ((storylines as any).sections as Array<{
                title: string;
                key_message: string;
              }>) || []
            ).map((sec, i) => (
              <div key={i} className="bg-[#0f0f0f] rounded-lg px-3 py-2.5">
                <p className="text-[11px] font-medium text-[#ededed] mb-0.5">
                  {sec.title}
                </p>
      ...
      {bidPrice && (bidPrice as any).total_cost && (
        <Section title="예산 요약">
          <KV
            label="총 예산"
            value={`${Number((bidPrice as any).total_cost).toLocaleString()}원`}
            cls="text-[#3ecf8e] font-bold"
          />
        </Section>
      )}
    </div>
      ...
      {results && results.length > 0 ? (
        <div className="space-y-2">
          {results.map((r, i) => (
            <div
              key={i}
              className="border border-[#262626] rounded-xl px-3.5 py-2.5"
            >
...
function BidPlanContent({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div>
      ...
      {strategy && (
        <Section title="가격 전략">
          <p className="text-xs text-[#ededed] leading-relaxed">{strategy}</p>
        </Section>
      )}
      <Section title="비용 구성">
        {laborCost && (
          <KV label="인건비" value={`${laborCost.toLocaleString()}원`} />
        )}
        {directCost && (
          <KV label="직접경비" value={`${directCost.toLocaleString()}원`} />
        )}
        {overhead && (
          <KV label="간접경비" value={`${overhead.toLocaleString()}원`} />
        )}
        {profit && <KV label="이윤" value={`${profit.toLocaleString()}원`} />}
      </Section>
      {costBasis && (
        <GenericContent data={costBasis as Record<string, unknown>} />
      )}
    </div>
  );
}
...
function ProposalContent({
  ...
  useEffect(() => {
    ...
    api.artifacts
      ...
      .then((a) => {
        ...
        const scores = d.section_scores as
          | Array<{ section: string; score: number; issue?: string }>
          | undefined;
  ...
  return (
    <div>
      {summary && (
        <Section title="요약">
          <p className="text-xs text-[#ededed] leading-relaxed">{summary}</p>
        </Section>
      )}
        ...
        <div className="space-y-2">
          {sections.map((sec, i) => {
            ...
            return (
              <div
                key={i}
                className={`border rounded-xl overflow-hidden ${issue ? "border-red-500/30" : "border-[#262626]"}`}
              >
                <button
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`text-[10px] font-bold rounded w-5 h-5 flex items-center justify-center shrink-0 ${
                        issue
                    ...
                    {issue && (
                      <span className="text-[8px] text-red-400 bg-red-500/10 rounded px-1.5 py-0.5 shrink-0">
                        {issue.score}점
                      </span>
                    )}
                  </div>
                  <svg
                    className={`w-3.5 h-3.5 text-[#5c5c5c] transition-transform shrink-0 ${expandedIdx === i ? "rotate-180" : ""}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
                ...
                {expandedIdx === i && (
                  <div className="px-3.5 pb-3 border-t border-[#1c1c1c]">
                    ...
                    <div className="pt-2 text-xs text-[#ededed] leading-relaxed whitespace-pre-wrap">
                      {sec.content}
                    </div>
                  </div>
                )}
...
function PptStoryboardContent({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div>
      {strategy && (
        <Section title="발표 전략" variant="success">
          <p className="text-xs text-[#ededed] leading-relaxed">{strategy}</p>
        </Section>
      )}
      {totalSlides && (
        <div className="mb-3">
          <KV label="슬라이드 수" value={`${totalSlides}장`} />
        </div>
      )}
          ...
          {slides.map((s) => (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[10px] font-bold text-indigo-400 bg-indigo-500/10 rounded w-5 h-5 flex items-center justify-center shrink-0">
                  {s.slide_no}
                </span>
...
function SelfReviewContent({ data }: { data: Record<string, unknown> }) {
  ...
  const axes = [
    {
      label: "컴플라이언스",
      score: data.compliance_score as number | undefined,
      max: 25,
    },
    {
      label: "전략 일관성",
      score: data.strategy_score as number | undefined,
      max: 25,
    },
    {
      label: "작성 품질",
      score: data.quality_score as number | undefined,
      max: 25,
    },
    {
      label: "근거 신뢰성",
      score: (data.trustworthiness as Record<string, unknown>)
          ...
          return (
            <div key={a.label}>
              ...
              <div className="h-2 bg-[#1c1c1c] rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${bc} transition-all duration-500`}
                  style={{ width: `${pct}%` }}
                />
              </div>
            </div>
...
function GenericContent({ data }: { data: Record<string, unknown> }) {
  ...
  if (entries.length === 0)
    return (
      <p className="text-xs text-[#5c5c5c] text-center py-8">데이터 없음</p>
    );
