    ...
    <div className="px-4 py-1.5 overflow-hidden">
      <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none">
        ...
        {files.map((f) => (
          ...
          >
            <span
              className={`w-4 h-4 rounded flex items-center justify-center text-[8px] font-bold ${fileColor(f.file_type ?? undefined)}`}
            >
        ...
        {ARTIFACT_DOCS.map((d) => {
          ...
          return (
            ...
            >
              <span
                className={`w-4 h-4 rounded flex items-center justify-center text-[8px] font-bold ${avail ? d.color : "text-[#3c3c3c] bg-[#1a1a1a]"}`}
              >
                {d.icon}
              </span>
              {d.label}
