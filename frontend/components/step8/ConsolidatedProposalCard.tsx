  ...
  if (error) {
    return (
      <div className="p-4 border border-red-200 rounded-lg bg-red-50">
        <p className="text-sm text-red-700">Failed to load consolidated proposal</p>
  ...
  if (!data) {
    return (
      <div className="p-4 border border-gray-200 rounded-lg bg-gray-50">
        <p className="text-sm text-gray-600">No consolidated proposal available</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 p-4 border rounded-lg bg-white">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">Consolidated Proposal</h3>
          <p className="text-sm text-gray-600 mt-1">Merged & Resolved Sections</p>
        </div>
        {onRevalidate && (
          <button
            onClick={onRevalidate}
      ...
      <div className="grid grid-cols-2 gap-4 py-3 border rounded-lg bg-gradient-to-r from-cyan-50 to-blue-50">
        <div>
          <p className="text-xs font-semibold text-gray-600 uppercase">Total Sections</p>
          <p className="text-3xl font-bold text-cyan-600 mt-2">{data.total_sections}</p>
        </div>
        <div>
          <p className="text-xs font-semibold text-gray-600 uppercase">Final Sections</p>
          <p className="text-3xl font-bold text-blue-600 mt-2">{data.final_sections.length}</p>
        </div>
      </div>

      {/* Executive Summary */}
      <div className="space-y-2">
        <h4 className="text-sm font-semibold text-gray-700">Executive Summary</h4>
        <div className="p-3 rounded-lg bg-blue-50 border border-blue-200">
          <p className="text-sm text-gray-800 leading-relaxed">{data.executive_summary}</p>
        </div>
      </div>

      {/* Final Section List */}
      <div className="space-y-2">
        <h4 className="text-sm font-semibold text-gray-700">Final Sections ({data.final_sections.length})</h4>
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-2">
            {data.key_changes.map((change, idx) => (
              ...
              >
                <span className="text-amber-600 font-semibold">{idx + 1}.</span> {change}
              </li>
            ))}
          </ul>
        </div>
      ...
      {data.section_lineage.length > 0 && (
        <div className="space-y-2">
          <h4 className="text-sm font-semibold text-gray-700">🔗 Section Lineage</h4>
          <div className="space-y-2 bg-gray-50 rounded-lg p-3 max-h-72 overflow-y-auto">
            {data.section_lineage.map((lineage: SectionLineage, idx: number) => (
              <div key={idx} className="space-y-1">
                ...
                {expandedLineage === idx && (
                  <div className="ml-4 p-3 bg-blue-50 border border-blue-200 rounded-lg space-y-2">
                    <div>
                      <p className="text-xs font-semibold text-blue-700 uppercase">Merged Into</p>
                      <p className="text-sm text-gray-900 mt-1">
                        <span className="font-medium">{lineage.merged_into_title}</span>
                        <span className="text-gray-600 ml-2">(index {lineage.merged_into_index})</span>
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-blue-700 uppercase">Conflict Resolution</p>
      ...
      <div className="p-4 rounded-lg bg-green-50 border border-green-200">
        ...
        <p className="text-sm text-gray-900 mt-2">
          ✓ Proposal successfully consolidated with {data.section_lineage.length} merge{" "}
