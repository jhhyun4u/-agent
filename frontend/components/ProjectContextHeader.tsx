...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-medium ${cfg.bg} ${cfg.text} border ${cfg.border} ${cfg.pulse ? "animate-pulse" : ""}`}
    >
      <span
  ...
  return (
    <header className="bg-[#111111] border-b border-[#262626] shrink-0 z-20">
          ...
          {moreOpen && (
            <div className="absolute right-0 mt-1 w-44 bg-[#1c1c1c] border border-[#262626] rounded-xl shadow-xl overflow-hidden z-30">
              <button
                onClick={() => {
                  setMoreOpen(false);
                  window.open(`/editor/${proposalId}`, "_blank");
                }}
              ...
              <div className="border-t border-[#262626]">
                <p className="px-3 py-1.5 text-[10px] text-[#5c5c5c] uppercase tracking-wider">
                  버전
                </p>
                {versions.map((v, idx) => (
                  <button
      ...
      <div className="flex items-center gap-2 px-4 pb-2.5 overflow-x-auto scrollbar-none">
        ...
        {dday && (
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium bg-[#1c1c1c] border border-[#262626] shrink-0 ${dday.color}`}
          >
            <svg
              className="w-3 h-3"
              fill="none"
            ...
            {dday.text}
            {deadlineDate && (
              <span className="text-[#8c8c8c] ml-0.5">({deadlineDate})</span>
            )}
          </span>
        )}
        ...
        {pos && (
          <span
            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-medium bg-[#1c1c1c] border border-[#262626] shrink-0 ${pos.color}`}
          >
            {pos.icon} {pos.label}
          </span>
        )}
