  ...
  return (
    <tr className="border-b border-[#1f1f1f] hover:bg-[#1a1a1a] group">
      <td className="py-1.5 px-2">
        <input value={item.grade} onChange={(e) => onChange(index, "grade", e.target.value)}
          className="w-full bg-transparent text-xs text-[#ededed] outline-none" />
      </td>
      <td className="py-1.5 px-2">
        <input value={item.role || ""} onChange={(e) => onChange(index, "role", e.target.value)}
          className="w-full bg-transparent text-xs text-[#ededed] outline-none" />
      </td>
      <td className="py-1.5 px-2">
        <input type="number" value={item.monthly_rate}
          onChange={(e) => onChange(index, "monthly_rate", parseInt(e.target.value) || 0)}
          className="w-full bg-transparent text-xs text-[#ededed] text-right outline-none font-mono" />
      </td>
      <td className="py-1.5 px-2">
        <input type="number" step="0.5" value={item.mm}
          onChange={(e) => onChange(index, "mm", parseFloat(e.target.value) || 0)}
          className="w-full bg-transparent text-xs text-[#ededed] text-center outline-none font-mono" />
      </td>
      <td className="py-1.5 px-2 text-right text-xs font-mono text-[#ededed]">{fmtWon(item.subtotal)}</td>
      <td className="py-1.5 px-1 text-center">
        <button onClick={() => onRemove(index)} className="text-red-400/30 group-hover:text-red-400/70 hover:!text-red-400 text-xs">✕</button>
      </td>
    </tr>
  );
}
...
export default function CostSheetEditor({ proposalId }: Props) {
  ...
  const [meta, setMeta] = useState({ project_name: "", client: "", proposer_name: "", cost_standard: "KOSA" });
  ...
  const [ratios, setRatios] = useState({ labor: 60, expense: 15, overhead: 18, profit: 7 });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setLabor(draft.labor_breakdown.map((l) => ({
          ...l, subtotal: l.subtotal || Math.round(l.monthly_rate * l.mm),
        })));
  ...
  // ── 자동 합계 ──
  const laborTotal = useMemo(() => labor.reduce((s, l) => s + l.subtotal, 0), [labor]);
  const expenseTotal = useMemo(() =>
    expenseGroups.reduce((s, g) => s + g.subitems.reduce((ss, e) => ss + e.amount, 0), 0),
    [expenseGroups]
  );
  const overheadTotal = Math.round(laborTotal * overheadRate / 100);
  const profitTotal = Math.round((laborTotal + overheadTotal) * profitRate / 100);
  ...
  const applyRatioDistribution = useCallback(() => {
    ...
    if (expenseTotal > 0 && expenseGroups.length > 0) {
      ...
      setExpenseGroups((prev) => prev.map((g) => ({
        ...g,
        subitems: g.subitems.map((e) => ({ ...e, amount: Math.round(e.amount * scale) })),
      })));
    }
    ...
    const targetLaborPlusOverhead = Math.round(baseAmount * (ratios.labor + ratios.overhead) / 100);
  ...
  // ── 인건비 편집 ──
  const handleLaborChange = useCallback((i: number, field: keyof LaborItem, value: string | number) => {
    setLabor((prev) => {
      const next = [...prev];
  ...
  const addLabor = () => setLabor((p) => [...p, { grade: "중급", role: "", monthly_rate: 5_170_000, mm: 1, subtotal: 5_170_000 }]);
  const removeLabor = (i: number) => setLabor((p) => p.filter((_, idx) => idx !== i));

  // ── 경비 그룹 편집 ──
  const addExpenseGroup = () => setExpenseGroups((p) => [...p, {
    name: "새 항목",
    subitems: [{ name: "", amount: 0, basis: "" }],
  }]);
  const removeExpenseGroup = (gi: number) => setExpenseGroups((p) => p.filter((_, i) => i !== gi));
  const updateGroupName = (gi: number, name: string) => {
    setExpenseGroups((p) => { const n = [...p]; n[gi] = { ...n[gi], name }; return n; });
  };
  const addSubitem = (gi: number) => {
    setExpenseGroups((p) => {
      const n = [...p];
      n[gi] = { ...n[gi], subitems: [...n[gi].subitems, { name: "", amount: 0, basis: "" }] };
      return n;
    });
  };
  ...
  const updateSubitem = (gi: number, si: number, field: keyof ExpenseItem, value: string | number) => {
    setExpenseGroups((p) => {
      const n = [...p];
  ...
  const flatExpenses: ExpenseItem[] = useMemo(() =>
    expenseGroups.flatMap((g) =>
      g.subitems.map((s) => ({
        name: g.subitems.length > 1 ? `${g.name} - ${s.name}` : (s.name || g.name),
        amount: s.amount,
        basis: s.basis,
      }))
    ), [expenseGroups]);
  ...
  if (loading) return <div className="text-sm text-[#8c8c8c] p-4">산출내역서 초안 로딩 중...</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-base font-semibold text-[#ededed]">산출내역서 편집</h2>
          <p className="text-xs text-[#8c8c8c] mt-0.5">항목을 수정하면 합계가 자동 재계산됩니다</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-[#8c8c8c]">{meta.cost_standard} · {totalMM.toFixed(1)}MM</span>
        </div>
      </div>
      ...
      {ratioMode && (
        <div className="rounded-lg border border-[#3ecf8e]/20 bg-[#3ecf8e]/5 p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-medium text-[#3ecf8e]">비목별 비율 조정</h3>
            <div className="flex items-center gap-2">
              <label className="text-[10px] text-[#8c8c8c]">목표 총액 (VAT 포함)</label>
              <input
                type="number"
                value={targetTotal}
          ...
          <div className="grid grid-cols-4 gap-3">
            {([
              { key: "labor" as const, label: "직접인건비", color: "bg-blue-400" },
              { key: "expense" as const, label: "직접경비", color: "bg-amber-400" },
              { key: "overhead" as const, label: "간접비", color: "bg-purple-400" },
              { key: "profit" as const, label: "기술료", color: "bg-green-400" },
            ]).map(({ key, label, color }) => (
              <div key={key}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] text-[#8c8c8c]">{label}</span>
                  <span className="text-xs font-mono text-[#ededed]">{ratios[key]}%</span>
                </div>
                <input
                  type="range" min="0" max="80" value={ratios[key]}
                  onChange={(e) => setRatios((p) => ({ ...p, [key]: parseInt(e.target.value) }))}
                  className="w-full h-1.5 rounded-full appearance-none cursor-pointer accent-[#3ecf8e]"
                  style={{ background: `linear-gradient(to right, ${color.replace("bg-", "rgb(")}, 50%) ${ratios[key]}%, #262626 ${ratios[key]}%)` }}
                />
                <div className="flex justify-between text-[9px] text-[#555] mt-0.5">
                  <span>현재 {actualRatios[key]}%</span>
                  <span>{targetTotal > 0 ? fmtShort(Math.round(targetTotal / 1.1 * ratios[key] / 100)) : "—"}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-[#8c8c8c]">
              합계: {ratios.labor + ratios.expense + ratios.overhead + ratios.profit}%
              {ratios.labor + ratios.expense + ratios.overhead + ratios.profit !== 100 && (
                <span className="text-amber-400 ml-1">(100%가 아님)</span>
              )}
            </span>
            <button
              onClick={applyRatioDistribution}
              disabled={!targetTotal || ratios.labor + ratios.expense + ratios.overhead + ratios.profit === 0}
      ...
      <div className="grid grid-cols-5 gap-2">
        {[
          ...
          <div key={label} className="rounded-lg border border-[#262626] bg-[#161616] p-3 text-center">
            <p className="text-[10px] text-[#8c8c8c]">{label}</p>
            <p className={`text-sm font-mono font-medium mt-1 ${label.includes("합계") ? "text-[#3ecf8e]" : "text-[#ededed]"}`}>
              {fmtShort(value)}
            </p>
      ...
      <div className="rounded-lg border border-[#262626] bg-[#161616] overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-[#262626]">
          ...
          <button onClick={addLabor} className="text-[10px] text-[#3ecf8e] hover:text-[#3ecf8e]/80">+ 인력 추가</button>
        </div>
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#262626] text-[#8c8c8c]">
              ...
              <th className="py-2 px-2 text-right font-normal w-[20%]">월단가</th>
          ...
          <tbody>
            {labor.map((item, i) => (
              <LaborRow key={i} item={item} index={i} onChange={handleLaborChange} onRemove={removeLabor} />
            ))}
          </tbody>
          <tfoot>
            <tr className="border-t border-[#333] bg-[#1a1a1a]">
              <td colSpan={3} className="py-2 px-2 text-xs font-medium text-[#ededed]">합계</td>
              <td className="py-2 px-2 text-center text-xs font-mono text-[#ededed]">{totalMM.toFixed(1)}</td>
              <td className="py-2 px-2 text-right text-xs font-mono font-medium text-[#ededed]">{fmtWon(laborTotal)}</td>
              <td></td>
            </tr>
          </tfoot>
        </table>
      ...
      <div className="rounded-lg border border-[#262626] bg-[#161616] overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-[#262626]">
          ...
          <button onClick={addExpenseGroup} className="text-[10px] text-[#3ecf8e] hover:text-[#3ecf8e]/80">+ 항목 추가</button>
        </div>
              ...
              <div key={gi} className="px-4 py-2">
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  <input
                    ...
                    placeholder="항목 분류명"
                  />
                  <span className="text-xs font-mono text-[#8c8c8c]">{fmtWon(groupTotal)}</span>
                  <button onClick={() => addSubitem(gi)} className="text-[9px] text-blue-400/70 hover:text-blue-400">+세부</button>
                  <button onClick={() => removeExpenseGroup(gi)} className="text-[9px] text-red-400/30 hover:text-red-400">✕전체</button>
                </div>
                {/* 세부항목 */}
                <div className="pl-4 space-y-1">
                  {group.subitems.map((sub, si) => (
                    <div key={si} className="flex items-center gap-2 group">
                      ...
                      <input
                        value={sub.name}
                        onChange={(e) => updateSubitem(gi, si, "name", e.target.value)}
                        placeholder="세부항목명"
                      ...
                      <input
                        type="number"
                        value={sub.amount}
                        onChange={(e) => updateSubitem(gi, si, "amount", parseInt(e.target.value) || 0)}
                        className="w-[25%] bg-transparent text-xs text-[#ededed] text-right outline-none font-mono"
                      />
                      <input
                        value={sub.basis}
                        onChange={(e) => updateSubitem(gi, si, "basis", e.target.value)}
                        placeholder="산출근거"
        ...
        <div className="flex items-center justify-between px-4 py-2 border-t border-[#333] bg-[#1a1a1a]">
          ...
          <span className="text-xs font-mono font-medium text-[#ededed]">{fmtWon(expenseTotal)}</span>
        </div>
      </div>

      {/* ═══ 3. 간접비·기술료 ═══ */}
      <div className="rounded-lg border border-[#262626] bg-[#161616] p-4">
        <h3 className="text-xs font-medium text-[#ededed] mb-3">3. 간접비 및 기술료</h3>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-[10px] text-[#8c8c8c]">간접비율</label>
            <div className="flex items-center gap-2 mt-1">
              <input
                type="range" min="0" max="200" value={overheadRate}
              ...
              />
              <input
                type="number" min="0" max="200" value={overheadRate}
            ...
            <p className="text-[10px] text-[#555] mt-1">직접인건비({fmtShort(laborTotal)}) × {overheadRate}% = {fmtShort(overheadTotal)}</p>
          </div>
          <div>
            <label className="text-[10px] text-[#8c8c8c]">기술료율</label>
            <div className="flex items-center gap-2 mt-1">
              <input
                type="range" min="0" max="50" value={profitRate}
              ...
              />
              <input
                type="number" min="0" max="50" value={profitRate}
                onChange={(e) => setProfitRate(parseInt(e.target.value) || 0)}
                className="w-14 px-1.5 py-1 bg-[#0a0a0a] border border-[#333] rounded text-xs text-[#ededed] text-right font-mono focus:border-[#3ecf8e] outline-none"
              />
              <span className="text-[10px] text-[#8c8c8c] w-2">%</span>
            </div>
            <p className="text-[10px] text-[#555] mt-1">(인건비+간접비)({fmtShort(laborTotal + overheadTotal)}) × {profitRate}% = {fmtShort(profitTotal)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between pt-2">
        <div>
          <p className="text-xs text-[#8c8c8c]">
            합계: <span className="text-[#3ecf8e] font-mono font-medium text-sm">{fmtWon(grandTotal)}</span>
            <span className="ml-2 text-[#555]">(VAT {fmtWon(vat)})</span>
          </p>
          <p className="text-[10px] text-[#555] mt-0.5">
            인건비 {actualRatios.labor}% · 경비 {actualRatios.expense}% · 간접비 {actualRatios.overhead}% · 기술료 {actualRatios.profit}%
          </p>
        </div>
        <button
