...
export default function SensitivityChart({ points, optimalRatio }: Props) {
  ...
  const scaleX = (r: number) =>
    PAD.left + ((r - minR) / (maxR - minR || 1)) * plotW;
  const scaleY = (prob: number) => PAD.top + plotH - (prob / maxProb) * plotH;

  // 확률 라인
  const probLine = points
    .map((p) => `${scaleX(p.ratio)},${scaleY(p.win_prob)}`)
    .join(" ");
  // 면적 채움
  const areaPath =
    `M ${scaleX(points[0].ratio)},${PAD.top + plotH} ` +
    points.map((p) => `L ${scaleX(p.ratio)},${scaleY(p.win_prob)}`).join(" ") +
    ` L ${scaleX(points[points.length - 1].ratio)},${PAD.top + plotH} Z`;

  // 최적점
  const optPt = points.reduce(
    (best, p) =>
      Math.abs(p.ratio - optimalRatio) < Math.abs(best.ratio - optimalRatio)
        ? p
        : best,
    points[0],
  );
  ...
  return (
    <div className="rounded-lg border border-[#262626] bg-[#161616] p-4 space-y-2">
      ...
      >
        ...
        {/* 최적점 */}
        <circle
          cx={scaleX(optPt.ratio)}
          cy={scaleY(optPt.win_prob)}
          r={5}
          fill="#f59e0b"
          stroke="#fff"
          strokeWidth={1.5}
        />
        <text
          x={scaleX(optPt.ratio)}
          y={scaleY(optPt.win_prob) - 10}
          fill="#f59e0b"
          fontSize={10}
          textAnchor="middle"
        >
