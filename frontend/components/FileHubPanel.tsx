...
const GENERATED_DOCS = [
  ...
  },
  {
    ...
    label: "제안서.hwpx",
    icon: "HWPX",
    color: "text-[#3ecf8e] bg-[#3ecf8e]/15",
  },
  {
    key: "pptx",
    label: "발표자료.pptx",
...
export default function FileHubPanel({
  ...
  // AI 산출물 존재 여부
  const [availableArtifacts, setAvailableArtifacts] = useState<Set<string>>(
    new Set(),
  );
  ...
  // 생성 문서 존재 여부
  const [generatedAvailable, setGeneratedAvailable] = useState<Set<string>>(
    new Set(),
  );
  ...
  useEffect(() => {
    ...
    const checkArtifacts = async () => {
      ...
      await Promise.allSettled(
        ARTIFACT_DOCS.map(async (d) => {
          try {
            await api.artifacts.get(proposalId, d.key);
            avail.add(d.key);
          } catch {}
        }),
      );
    ...
    const loadFiles = async () => {
      ...
      try {
        setFiles((await api.proposals.listFiles(proposalId)).files);
      } catch {
      } finally {
      ...
      for (const g of GENERATED_DOCS) {
        try {
          const res = await fetch(
            `${baseUrl}/proposals/${proposalId}/download/${g.key}`,
            { method: "HEAD" },
          );
  ...
  const filteredArtifacts = ARTIFACT_DOCS.filter(
    (d) => !q || d.label.toLowerCase().includes(q),
  );
  const filteredFiles = files.filter(
    (f) => !q || f.filename.toLowerCase().includes(q),
  );
  const filteredDocs = submissionDocs.filter(
    (d) => !q || d.doc_type.toLowerCase().includes(q),
  );
  const filteredGenerated = GENERATED_DOCS.filter(
    (g) => !q || g.label.toLowerCase().includes(q),
  );

  const availCount = availableArtifacts.size;
  const totalCount = ARTIFACT_DOCS.length;
  const docsCompleted = submissionDocs.filter(
    ...
    <div className="space-y-1">
      ...
      <div className="px-2 pt-2 pb-1">
        <div className="relative">
          <svg
            className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#5c5c5c]"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
        ...
        <div className="space-y-0.5">
          {filteredArtifacts.map((d) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-5 h-5 rounded flex items-center justify-center text-[9px] font-bold shrink-0 ${avail ? d.color : "text-[#3c3c3c] bg-[#1a1a1a]"}`}
                >
                  {d.icon}
                </span>
                ...
                {avail && (
                  <svg
                    className="w-3 h-3 text-[#5c5c5c] shrink-0"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2}
      ...
      >
        ...
        <div className="space-y-0.5">
          {filteredFiles.map((f) => (
            ...
            >
              <span
                className={`w-5 h-5 rounded flex items-center justify-center text-[8px] font-bold shrink-0 ${fileColor(f.file_type ?? undefined)}`}
              >
      ...
      <Section
        ...
        badge={
          submissionDocs.length > 0
            ? `${docsCompleted}/${submissionDocs.length} 완료`
            : "0"
        }
      ...
      >
        <div className="space-y-0.5">
          {filteredDocs.length === 0 && !docsLoading && (
            <p className="px-3 py-2 text-[11px] text-[#5c5c5c]">
              제출서류가 아직 추출되지 않았습니다.
            </p>
          )}
          {filteredDocs.map((d) => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 text-left truncate text-[#ededed]">
                  {d.doc_type}
                </span>
                <span className="text-[10px] text-[#5c5c5c] shrink-0">
                  {d.required_format}
                </span>
                <span className={`text-[10px] shrink-0 ${st.color}`}>
                  {st.label}
                </span>
                {hasFile && (
                  <button
                    onClick={() => {
                      ...
                      window.open(
                        `${baseUrl}/proposals/${proposalId}/submission-docs/${d.id}/download`,
                        "_blank",
                      );
                    }}
        ...
        <div className="space-y-0.5">
          {filteredGenerated.map((g) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-5 h-5 rounded flex items-center justify-center text-[8px] font-bold shrink-0 ${avail ? g.color : "text-[#3c3c3c] bg-[#1a1a1a]"}`}
                >
                  {g.icon}
                </span>
                ...
                {avail ? (
                  <>
                    ...
                    <svg
                      className="w-3.5 h-3.5 text-[#8c8c8c] shrink-0"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
